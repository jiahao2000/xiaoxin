...
Page({
  ...
  getDataFromOneNet: function () {
    ...
    const requestTask = wx.request({
      url: 'https://api.heclouds.com/devices/503153817/datapoints?datastream_id=Light,hongwai&limit=1 ',//改为自己的设备ID
      header: {
        'content-type': 'application/json',
        'api-key': 'NwaqZjj66qjtUk=vAjYoHWgozc0='//改为自己的APIkey
      },
      success: function (res) {
        ...
        //通过逻辑改变数字的显示状态
        var Light = (light > 100)
        var hongwai = Number(human)

        console.log("Light : " + Light)
        console.log("hongwai : " + hongwai)
        if (hongwai && Light) {
          console.log("!!!")
          that.setData({
            upChecked: true
          })
        } else if (hongwai && !Light) {
          that.setData({
            sleepChecked: true
          })
        } else if (!hongwai && Light) {
